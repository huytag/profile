...
import { canViewReport, isOutlet } from "../../../services/hasPermission";
import { PER_TRADE_DISPLAY_REPORT_VIEW } from "../../../utils/enumPermission";

const CampaignDisplayItemComponent = ({ item }) => {
  return (
    <>
      <Link
        to={`${item?.id}/${
            ...
            <div style={{ width: `calc(100% - 29vw)` }} className="ml-auto">
              {isOutlet() && (
                <TextBaseLineComponent
                  title="Trạng thái:"
                  value={
                    item?.display_app_result?.status_text ?? "Chưa tham gia"
                  }
                />
              )}
...
export default CampaignDisplayItemComponent;
