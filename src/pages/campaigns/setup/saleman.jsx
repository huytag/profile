...
const CampaignSetUpSaleman = () => {
  ...
  const fetchStatictis = async () => {
    setLoading(true);
    await apiClient
      .get(`${SETUP_DETAIL}/${id}/statistic`)
      .then((response) => {
        setStatistic(response?.data?.setup_statistic);
      })
  ...
  const fetchOutlets = async () => {
    setLoading(true);
    await apiClient
      .get(`${SETUP_DETAIL}/${id}/outlets`, { params: { ...filter } })
      .then((response) => {
        setData(response?.data?.setup_outlets);
      })
      .catch((error) => noticeError(error?.message))
      .finally(() => setLoading(false));
  };
  ...
  return (
    <Page className="page " style={{ paddingTop: "60px" }} hideScrollbar={true}>
      ...
      <Box className="px-5 py-3 text-4xl items-center mt-4 bg-white rounded-xl shadow-lg shadow-black-500/50">
        ...
        <CampaignOutletComponent list={data} appName="setup" />
      </Box>
    </Page>
  );
};

export default CampaignSetUpSaleman;
